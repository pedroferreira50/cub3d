...
void *mlx_int_parse_xpm(mlx_ptr_t *xvar, void *info, int info_size, char *(*f)())
{
  ...
  if (!(line = f(info, &pos, info_size)) ||
      !(tab = mlx_int_str_to_wordtab(line)) || !(cols = atoi(tab[0])) ||
      !(rows = atoi(tab[1])) || !(nc = atoi(tab[2])) ||
      !(cpp = atoi(tab[3])))
    RETURN;
  free(tab);
  tab = 0;
...
{
  if (!(clip_data = malloc(4*cols*rows)) ||   // ok, nice size ..
      !(clip_img = XCreateImage(xvar->display, xvar->visual,
        1, XYPixmap, 0, clip_data,
        cols, rows, 8, (cols+7)/8)) )
    RETURN;
  memset(clip_data, 0xFF, 4*cols*rows);
}
    */
    if (method)
  ...
  if (!(img = mlx_new_image(xvar, cols, rows)))
    RETURN;
  // opp = img->bpp/8;
  opp = 4;
  ...
  while (i--)
  {
    ...
    while (x < cols)
    {
      ...
      //	  if (col==-1)
      //	    XPutPixel(clip_img, x, rows-1-i, 0);
      //	  else
      if (col == -1)
        col = 0xFF000000;
...
void *mlx_xpm_file_to_image(mlx_ptr_t *xvar, char *file, int *cols, int *rows)
{
  int fd;
  int size;
  char *ptr;
...
void *mlx_xpm_to_image(mlx_ptr_t *xvar, char **xpm_data, int *cols, int *rows)
{
  mlx_img_list_t *img;
