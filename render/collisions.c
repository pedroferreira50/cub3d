...
float	calculate_x_intercept(t_player *player, t_map *map, float angle)
{
	int	tile_x;

	tile_x = floor(player->x / map->tile_size) * map->tile_size;
	if (is_facing_west(angle))
		...
		t_cast *h)
{
	...
	step = init_ray_step(player->x + (calculate_y_intercept(player, map, angle)
				- player->y) / tan(angle), calculate_y_intercept(player, map,
				angle), x_step, y_step);
		...
		t_cast *v)
{
	...
	step = init_ray_step(calculate_x_intercept(player, map, angle), player->y
			+ (calculate_x_intercept(player, map, angle) - player->x)
			* tan(angle), x_step, y_step);
	step.horizontal_dir = horizontal_dir;
	trace_vertical_ray(map, &step, v);
}
