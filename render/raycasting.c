...
/*   Updated: 2025/07/13 13:33:28 by gro-donn         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	cast_ray(t_cub_elements *app, float angle, int col, t_ray *rays)
{
	...
	if (!h.hitted)
		...
		h.distance = hit_distance(app->player.x, app->player.y, h.hit[X],
				h.hit[Y]) * fish_eye;
	if (!v.hitted)
		v.distance = INFINITY;
	else
		v.distance = hit_distance(app->player.x, app->player.y, v.hit[X],
				v.hit[Y]) * fish_eye;
...
void	raycasting(t_cub_elements *app, t_ray *rays)
{
	...
	angle = app->player.angle - HALF_FOV;
	for (int col = 0; col < app->mlx.width; col++, angle
		+= app->player.ray_step)
	{
